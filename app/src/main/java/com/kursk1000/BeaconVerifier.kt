 ...
 * отклоняем как повтор записанного ранее пакета. Карта живёт в инстансе верификатора
 * (его владеет ViewModel), [reset] зовётся там же, где сканер «забывает» эфир (закрытие
 * карточки / стоп скана) — иначе легальный повторный подход к метке посчитался бы replay.
 */
class BeaconVerifier(
    ...
    /** Забыть историю принятых счётчиков — вызывать при «забывании» эфира (dismiss/стоп скана). */
