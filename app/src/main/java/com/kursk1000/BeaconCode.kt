...
/** Динамический код метки (общий модуль без Android-зависимостей для эмулятора и гида):
Каждые TIME_STEP_MS меняется счётчик counter = epochMillis / TIME_STEP_MS.
Код = усечённый до CODE_LEN байт HMAC‑SHA256(секрет, counter).
Гид вычисляет свой HMAC и сравнивает – это даёт защиту от повторов (код протухает за 1–2 шага) и клонирования (без секрета будущие коды недоступны).
Wire‑формат BLE Service Data: [VERSION][CODE_LEN байт HMAC]. */
object BeaconCode {

    /** Шаг ротации кода */
    const val TIME_STEP_MS = 30_000L

    /** Сколько байт HMAC оставляем в эфире */
    ...
    /** Достать код из пакета Service Data, проверив версию и длину. `null` — пакет не наш */
